...
app =   Flask(__name__)
app.config["SECRET_KEY"] = "jhagfdjkafbk"
...
def disconnect():
    ...
    print(f"{name} has left room {room}. Total members: {rooms[room]['members'] if room in rooms else 0}")      
...
def message(data):
    ...
    print(f"Message from {content['name']} in room {room}: {content['message']}")
